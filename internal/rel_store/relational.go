...
func (r *relationalDB) getFedRAMPRequirement(ctx context.Context, id string) (*fedramp.Requirement, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, category, name, statement, keyword, version FROM fedramp_requirements WHERE id = ?`, id)
	var req fedramp.Requirement
	if err := row.Scan(&req.ID, &req.Category, &req.Name, &req.Statement, &req.Keyword, &req.Version); err == sql.ErrNoRows {
		return nil, fmt.Errorf("FedRAMP requirement %q not found", id)
	} else if err != nil {
		return nil, err
	}
	return &req, nil
